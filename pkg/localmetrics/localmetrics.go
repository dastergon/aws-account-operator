...
type MetricsCollector struct {
	...
	accountClaimPendingDuration     *prometheus.HistogramVec
	ccsAccountClaimPendingDuration  *prometheus.HistogramVec
...
func NewMetricsCollector(store cache.Cache) *MetricsCollector {
	return &MetricsCollector{
		...
		accountClaimPendingDuration: prometheus.NewHistogramVec(prometheus.HistogramOpts{
		...
		ccsAccountClaimPendingDuration: prometheus.NewHistogramVec(prometheus.HistogramOpts{
...
func (c *MetricsCollector) SetAccountClaimPendingDuration(ccs bool, accountClaim string, duration float64) {
	if ccs {
		c.ccsAccountClaimPendingDuration.WithLabelValues(accountClaim).Observe(duration)
	} else {
		c.accountClaimPendingDuration.WithLabelValues(accountClaim).Observe(duration)
	}
}
